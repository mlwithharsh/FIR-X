...
// Robust template generator with comprehensive data replacement
export class RobustGenerator {
  // Comprehensive replacement mappings based on typical DAR form content
  private static buildComprehensiveReplacements(data: ReportFormValues): Record<string, string> {
    ...
    return {
      ...
      "212-2026": caseDetails.fir_number.replace("/", "-"),
  ...
  private static getNatureLabel(type: string): string {
    return "Injury Fatal Damage / loss of the property Any other loss/injury";
  }
  ...
  static async generateDocument(data: ReportFormValues): Promise<Blob> {
    try {
      ...
      // Get template text for debugging
      const fullText = doc.getFullText();
      console.log('=== TEMPLATE TEXT ANALYSIS ===');
      console.log('Full template text length:', fullText.length);
      console.log('Sample template content:', fullText.substring(0, 500));
      console.log('=== END TEMPLATE ANALYSIS ===');

      // Build comprehensive replacements
      const replacements = this.buildComprehensiveReplacements(data);
      console.log('=== REPLACEMENTS BUILT ===');
      console.log('Number of replacements:', Object.keys(replacements).length);
      Object.entries(replacements).forEach(([key, value]) => {
        console.log(`"${key}" -> "${value}"`);
      });
      console.log('=== END REPLACEMENTS ===');
      ...
      try {
        ...
        throw new Error('Failed to render document with provided data');
      }
