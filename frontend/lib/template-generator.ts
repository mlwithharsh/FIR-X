...
// Template-based document generator that preserves exact formatting
export class TemplateGenerator {
  ...
  static async generateFromTemplate(data: ReportFormValues): Promise<Blob> {
    try {
      // Load the template from public directory
      ...
      // DEBUG: Log the full template content
      const fullText = doc.getFullText();
      console.log('=== TEMPLATE FULL TEXT ===');
      console.log(fullText);
      console.log('=== END TEMPLATE TEXT ===');

      // Get replacements and apply them
      const replacements = this.buildReplacements(data);
